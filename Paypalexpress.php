...
public function _makeOptionPaypalbutton() {
     	...
     	// Paypalcustomerdata is saved in database 
		//is executed after the payment	
			if(isset($_GET["firstname"])){		//checks if the url data is available
				...
				$cart = Tools_ShoppingCart::getInstance();// the actual order 
				
				
				
				//preperement of the customer array
				$customerData = array(
	   			 "firstname"    => $firstname,
	   			 "lastname"  => $lastname,
	   			 "email"  => $email,
				...
				$addressType = Models_Model_Customer::ADDRESS_TYPE_SHIPPING; //adresstype
				
			
				$this->_checkoutSession->initialCustomerInfo = $customerData;// the actual session
				$customer = Shopping::processCustomer($customerData);	//customer array is saved in database
				
							
					
				if ($customer->getId()) {
                    $customer->setAttribute('mobilecountrycode', $customerData['mobilecountrycode']);
                    Application_Model_Mappers_UserMapper::getInstance()->saveUserAttributes($customer); //customer data is saved
				...
				$addressId = Models_Mapper_CustomerMapper::getInstance()->addAddress($customer, $address, $addressType); //customer address is saved
				...
				$product->setPrice($this->calculatePaypalFee($paypalfee)-$summary['total']);		
